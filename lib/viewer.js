...
export class Viewer {
  ...
  /** Centered, face-on view of the current slice, looking down the time axis. */
  frontCamera() {
    const target = new THREE.Vector3(0, 0, this.desiredTargetZ);
    const distance = Math.abs(this.homeOffset.z);
    this.controls.target.copy(target);
    this.camera.position.set(0, 0, target.z - distance);
    this.camera.lookAt(target);
    this.controls.update();
  }
  ...
  dispose() {
    ...
    // dispose() frees three's own resources but leaves the WebGL context alive on
    // the canvas. React StrictMode mounts twice in development, so without this
    // the second Viewer would pick up the first one's torn-down context.
    try { this.renderer.forceContextLoss(); } catch { /* not always available */ }
  }
}
