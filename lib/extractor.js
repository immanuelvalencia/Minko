 ...
 */
export function volumeLayout(srcWidth, srcHeight, frameCount, budgetBytes, maxDim) {
  const depth = Math.min(frameCount, maxDim);
...
export async function extractVolume(probe, options = {}) {
  ...
  const layout = volumeLayout(probe.width, probe.height, totalFrames, budgetBytes, maxDim);
